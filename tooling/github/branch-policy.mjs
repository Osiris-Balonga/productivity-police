...
export function evaluateBranchPolicy(pullRequest) {
  ...
  if (base.ref === "dev") {
    ...
    const sameRepositoryDependabot = head.ref.startsWith("dependabot/") && base.repo.id === head.repo.id;
    const plannedBranch = allowedPrefixes.some((prefix) => head.ref.startsWith(prefix));
    return sameRepositoryMain || sameRepositoryDependabot || plannedBranch
