...
The `pull_request_target` workflow reads policy only from the trusted base commit. It never checks out, installs, builds, or executes code from a proposed branch. `dev` accepts branches with `feat/`, `fix/`, `chore/`, `docs/`, `test/`, `ci/`, `build/`, or `refactor/`. `main` accepts only same-repository `dev`.
